...
!
    ...
    real ::                 ct, ft, ctApprox, alpha1, beta1, sdBeta, ctprop, ftprop, delta, ffe, pi, dummy6(6), &
                            teller_c, dumE
    real, allocatable ::    priorMean(:), priorCov(:,:), relComp(:), drawsJU(:,:), drawsMU(:,:), &
...
!    
    ...
    delta = .1
    allocate(drawsJU(samsize,numcorr),drawsMU(samsize,numcorr),ones(samsize,1), statX(15,numcorrgroup), means(numcorrgroup,1), &
...
!
    ...
    !The computation of the relative complexity is an adaptation of a function
    !in BaIn by Xin Gu (Gu, Mulder, Hoijtink, in preparation).
!    
...
!       
    ...
    if(numcorr==1) then
        if(numIneq(h1)>0) then
            allocate(Rh1(numIneq(h1),numcorr),drawsTrans(samsize,numIneq(h1)),wh1(samsize,numIneq(h1)))
            wh1 = 0
            ...
            do s1 = 1,samsize
                do r1 = 1,numIneq(h1)
                    if(drawsTrans(s1,r1)>Rcon(teldummy+r1,numcorr+1)) then
                        wh1(s1,r1) = 1
                    end if
                end do
                if(product(wh1(s1,:))==1) then
                    teller_c = teller_c + 1
                end if
            end do
            rcI(h1,1) = teller_c/samsize
    ...
    if(numcorr>1) then
        do h1=1,numH
            ...
            if(numIneq(h1)>0 .and. numEq(h1)==0) then
                ...
                allocate(Rh1(numIneq(h1),numcorr+1))
                Rh1(1:numIneq(h1),1:(numcorr+1)) = Rcon((teldummy+1):(teldummy+numIneq(h1)),1:(numcorr+1))
                call compute_prob_bain(0,numIneq(h1),numcorr,Rh1(1:0,:),Rh1,meansAll(:,1),covmAll,dumE,rcI(h1,1))
                teldummy = teldummy + numIneq(h1)
                deallocate(Rh1)
            end if
            if(numIneq(h1)>0 .and. numEq(h1)>0) then
    ...
    !                
                ...
                Rh1IE(1:numIneq(h1),numcorr-numEq(h1)+1) = Rcon((teldummy+1):(teldummy+numIneq(h1)),numcorr+1) - &
                    wh1IE(:,1)
!                call compute_probability(numIneq(h1),numcorr-numEq(h1),Rh1IE,meanI,covmI,rcI(h1,1))              
    ...
    write(*,*)
!    
!    !compute relative complexity of the complement hypothesis
!    do h1=1,numH
!        if(numEq(h1)==0) then
!            rcI(numH+1,1) = rcI(numH+1,1)-rcI(h1,1)
!        end if
...
!
    !compute ML (or use least squares?) estimates
    do g1=1,numG
...
!   in BaIn by Xin Gu (Gu, Mulder, Hoijtink, in preparation).
!
    !compute relative fit based on posterior under the JU-prior and/or the MU-prior
    rfE = 1
    rfI = 1
!
...
!        numER = numEq(h)
!        numIR = numIneq(h)
!        allocate (Rr(numER+numIR,numcorr+1),ERr1(numER,numcorr+1),IRr(numIR,numcorr+1),constant(numIR), &
!              transR(numIR,numIR),transcon(numIR))!Allocate the augmented matrix [R|r]
!        ERr1(1:numER,1:(numcorr+1)) = Rcon((teldummy+1):(teldummy+numER),1:(numcorr+1))
!        teldummy = teldummy + numER
!        IRr(1:numIR,1:(numcorr+1)) = Rcon((teldummy+1):(teldummy+numIR),1:(numcorr+1))
!        teldummy = teldummy + numIR
!        constant(:) = IRr(:,numcorr+1)
!    !                 
!        call Mrank(numIR, numcorr, rowrank, IRr, transR, constant, transcon) 
!    !
!        do i = 1, numER+rowrank
!            if (i.LE.numER) then 
!               Rr(i,1:(numcorr+1))=ERr1(i,1:(numcorr+1))
!            else 
!               Rr(i,1:(numcorr+1))=IRr(i-numER,1:(numcorr+1))
!            end if
!        end do
!    !           
!        allocate (ff(numIR),numf(numIR),bet(numER+rowrank),betacov(numER+rowrank,numER+rowrank), &
!            tempmatrix(numER+rowrank,numcorr),invbetacov(numER+rowrank,numER+rowrank), &
!            invDiag(numER+rowrank,numER+rowrank),IE(numER+rowrank,numER+rowrank), &
!            B(numER+rowrank,numER+rowrank))  
!         
!        ! Parameter transformation: beta=Rrtheta-r  
!        call MURRV(Rr(1:(numER+rowrank), 1:(numcorr+1)), theta(1:(numcorr+1)), bet) ! beta=Rr*theta  
!        call MRRRR(Rr(1:(numER+rowrank), 1:numcorr), thetacov, tempmatrix)  ! tempmatrix=R*thetacov. Note that R=Rr(1:numR,1:numcorr): discard the last column. 
!        call MXYTF(tempmatrix, Rr(1:(numER+rowrank),1:numcorr), betacov)   ! betacov=tempmatrix*R^(T), then betacov=R*cov*R^(T)
!      !        
!        pi=3.1415926
!      !            
!        call LINRG(betacov,invbetacov)        ! Compute the inverse of betacov i.e., invbetacov=betacov^(-1)
!      !
!         !"invDiag" is the inverse matrix of the diagonal matrix of "invbetacov", i.e.,[diag(invbetacov)]^(-1)
!        IE=0
!        do i=1,(numER+rowrank)
!            invDiag(i,i)=(invbetacov(i,i))**(-1)
!            IE(i,i)=1
!        end do
!    !           
!       ! Compute B=I-[diag(invbetacov)]^(-1)*invbetacov, and also for the prior
!        do i=1,(numER+rowrank)
!            do j=1,(numER+rowrank)
!                B(i,j)=IE(i,j)-invDiag(i,i)*invbetacov(i,j)
!            end do
!        end do  
!    !          
!        if(numER>0) then
!            ffe=1/sqrt((2*pi)**(numER)*abs(det(betacov(1:numER,1:numER))))* &
!                exp(-0.5*BLINF(invbetacov(1:numER,1:numER),bet(1:numER),bet(1:numER)))
!            rfE(h,1) = ffe
!        end if
!    !
!        do k=1,numIR
!            call gibbssampler(k,numER,numIR,rowrank,bet,transcon,B,invbetacov,transR,ff(k),numf(k))
!            print '("inequality constraint", I4 " in hypothesis", I3  " under the posterior is evaluated", I10)', k, h
!        end do
!    !        
!        !relative fit under hypothesis is product of all separate fit elements
!        if(numIR>0) then
!            rfI(h,1) = product(ff)
!            end if
!        write(*,*)
!    !           
!      ! deallocate variable used in the previous hypothesis
!        deallocate(Rr,ff,numf,bet,betacov,tempmatrix,constant,transR,transcon, &
!          invbetacov,invDiag,IE,B,ERr1,IRr)
    end do
!    
...
!        write(60,*)corrMat(2,1),corrMat(3,1),corrMat(3,2),corrMat(4,1),corrMat(4,2),corrMat(4,3)
    end do
!    
end subroutine draw_JU


!!Draw rho's from marginally uniform distribution
!!Thus draw Sigma~inv-Wishart(I_P,P+1)
!!See Barnard, McCullogh, & Meng (2000)
!subroutine draw_MU(drawsCorr)
!!    
!    use IMSL_LIBRARIES
!    use global_variables
!    implicit none
...
!!    integer, intent (in) :: 
!    real, intent (out) :: drawsCorr(samsize,numcorrgroup)
!    integer ::            r1, r2, s1, i1, i2, k1, teldummy, irank
!    real ::               dummy1, corrMat(numcorrgroup,numcorrgroup), &
!                          RNmat(P+1,P), I_P(P,P), SigmaInv(P,P), &
!                          Sigma(P,P), sdvec(P), sigmaInvDiag(P,P), sigmavec(P)
!!
!    I_P = eye(P)
!!
!    do s1=1,samsize
!        !sample inverse Wishart distributed covariance matrices Sigma
!        call rnmvn(I_P, RNmat)
!        SigmaInv = RNmat.tx.RNmat
!        Sigma = .i.(SigmaInv)
!        telDummy = 0
!!
!        !convert (co)variances in Sigma to correlations
!        sigmavec = sqrt(diagonals(Sigma))
!        do i1=1,P-1
!            drawsCorr(s1,(teldummy+1):(teldummy+P-i1)) = Sigma(i1,(i1+1):P)/(sigmavec((i1+1):P)*sigmavec(i1))
!            telDummy = teldummy + P - i1
!        end do
!    end do
!!    write(*,*)drawsCorr(s1-1,:)
!!    pause
!!    
!end subroutine draw_MU
...
!
!!same subroutine as in BOCOR
!subroutine compute_probability(numI, dimI, Rineq, meanZ, covZ, probI)
!!
!    use imsl_libraries
!    use global_variables
!    implicit none
!!       
!    integer, intent(in) :: numI, dimI
!    real, intent(in)    :: meanZ(dimI,1), covZ(dimI,dimI), Rineq(numI,dimI+1)
!    real, intent(out)   :: probI
!    
!    real :: eigenvals(numI), probSub(numI), zmean, zsig2, dummy3(1,1), lowerbS, rnIPS, dummyDraw, &
!            lowerbIC, upperbIC, dummyUB, wvec(numI,1), machPres
!    real, allocatable :: dummy2(:,:), S12(:,:), S22(:,:), S22inv(:,:), probvec(:), ZS(:,:), &
!                         covTest(:,:), meanTest(:,:), Rineq2(:,:), RineqPerm(:,:), RineqSub(:,:), covSub(:,:), eigenSub(:), &
!                         oneRow(:), transMatrix(:,:), eigenDummy(:)
!    integer :: c1, i1, s1, z1, posthits, priorhits, numTrans, onegood, r1, lowerTrue, upperTrue,&
!               ic1, MatCorrTest(dimI,dimI), numCorrTest
!!
!!800 format(21F18.7)
!    machPres = 1e-6
!    dummyUB = 1e20
!    wvec(:,1) = Rineq(:,dimI+1)
!    allocate(probvec(samsize2),covTest(numI,numI), meanTest(numI,1))
!    covTest = matmul(matmul(Rineq(:,1:dimI),covZ),transpose(Rineq(:,1:dimI)))
!    meanTest = matmul(Rineq(:,1:dimI), meanZ)
!    call evlsf(covTest,eigenvals)
!    if(minval(eigenvals)>machPres) then !the transformation can be used
!        probSub(1) = 1-anordf((wvec(1,1)-meanTest(1,1))/sqrt(covTest(1,1)))
!        allocate(dummy2(numI-1,1), S12(1,numI-1), S22(numI-1,numI-1), S22inv(numI-1,numI-1),&
!                 ZS(numI,1))
!        do i1=2,numI
!            ZS(:,1) = meanTest(:,1)
!            probvec = 0
...
!                do z1=1,i1-1
!                    
!                    call compute_condMeanVar(z1,numI,meanTest(1:numI,1),covTest(1:numI,1:numI),ZS(1:numI,1),zmean,zsig2)
!                    
!                    !draw using inverse probability sampling
!                    lowerbS = (Rineq(z1,dimI+1) - zmean)/sqrt(zsig2) !lower bound on standardized scale
!                    call inverse_prob_sampling(zmean,zsig2,1,0,wvec(z1,1),dummyUB,ZS(z1,1))
!                end do
!                
!                call compute_condMeanVar(i1,numI,meanTest(1:numI,1),covTest(1:numI,1:numI),ZS(1:numI,1),zmean,zsig2)
!                
!                !compute conditional probability that Z(i1)>Rineq(i1,dimI+1)
!                probvec(s1) = 1-anordf((wvec(i1,1)-zmean)/sqrt(zsig2))
!                
!                !draw unconstrained
!                dummyDraw = rnnof()
!                ZS(i1,1) = dummyDraw * sqrt(zsig2) + zmean
!                    
!                do z1=i1+1,numI                  
!                    call compute_condMeanVar(z1,numI,meanTest(1:numI,1),covTest(1:numI,1:numI),ZS(1:numI,1),zmean,zsig2)
!                    dummyDraw = rnnof()
!                    ZS(z1,1) = dummyDraw*sqrt(zsig2) + zmean
!                end do
!            end do
!            probSub(i1) = sum(probvec)/real(samsize2)
!        end do
!    else
!        !The transformation cannot be used. Therefore a smaller transformation is used.
!        !Search for maximal number of transformations so that above method can be used,
!        !and use proportion of hits for remaining inequality constraints.
!        
!        !check which correlations are constrianed
!        MatCorrTest = 0
!        numCorrTest = 0
!        do i1=1,dimI
!            if(sum(abs(Rineq(:,i1)))>.0001) then
!                numCorrTest = numCorrTest + 1
!                MatCorrTest(numCorrTest,i1) = 1
!            end if
!        end do       
!!              
!        deallocate(covTest,meanTest)
!        allocate(covTest(numCorrTest,numCorrTest), meanTest(numCorrTest,1), &
!            Rineq2(numI,numCorrTest+1), dummy2(numCorrTest-1,1), S12(1,numCorrTest-1), &
!            S22(numCorrTest-1,numCorrTest-1), S22inv(numCorrTest-1,numCorrTest-1), &
!            ZS(numCorrTest,1), oneRow(numCorrTest+1), transMatrix(numCorrTest,numCorrTest), &
!            eigenDummy(numCorrTest))
!!
!        !First restrict problem to Zrho's that are inequality constrained.
!        !This is allowed because of assumed multivariate normality of Z_rho
!        covTest = matmul(matmul(MatCorrTest,covZ),transpose(MatCorrTest))
!        meanTest = matmul(MatCorrTest,meanZ)
!        Rineq2(:,1:numCorrTest) = matmul(Rineq(:,1:dimI),transpose(MatCorrTest))
!        Rineq2(:,numCorrTest+1) = Rineq(:,dimI+1)
!        
!        !Search for largest set of inequality constraints for which above method can be used.
!        !Start with first row of Rineq.
!        allocate(RineqPerm(numI,numCorrTest+1))
!        RineqPerm = Rineq2
!        
!        numTrans = 2 !at least first 2 inequality constraints are independent.
!        onegood = 1
!        do while(numTrans <= numCorrTest .and. onegood==1)
!        
!            allocate(RineqSub(numTrans+1,numCorrTest+1), covSub(numTrans+1,numTrans+1), eigenSub(numTrans+1))
!            RineqSub(1:numTrans,:) = RineqPerm(1:numTrans,:)
!            onegood = 0
!        
!            do r1=numTrans+1,numI
!                RineqSub(numTrans+1,:) = RineqPerm(r1,:)
!                covSub = matmul(matmul(RineqSub(:,1:numCorrTest),covTest),transpose(RineqSub(:,1:numCorrTest)))
!                call evlsf(covSub,eigenSub)
!                if(minval(eigenSub)>machPres) then !positive definite subtransformation
!                    oneRow(:) = RineqPerm(r1,:)
!                    RineqPerm(r1,:) = RineqSub(numTrans+1,:)
!                    RineqPerm(numTrans+1,:) = oneRow(:)
!                    numTrans = numTrans + 1
!                    onegood = 1
!                    deallocate(RineqSub, covSub, eigenSub)
!                    exit
!                end if
!            end do
!        end do
!!        
!        transMatrix(1:numTrans,1:numCorrTest) = RineqPerm(1:numTrans,1:numCorrTest)
!        do r1=numTrans+1,numCorrTest
!            transMatrix(r1,1:r1) = 1 !add rows that are independent of other rows to ensure one-to-one transformzation
!        end do
!!
!        !transform mean vector, covmatrix and inequality constraints such that efficient method can be used for
!        !evaluating the first 'numTrans' inequality constraints.
!        meanTest = matmul(transMatrix,meanTest)
!        covTest = matmul(matmul(transMatrix,covTest),transpose(transMatrix))
!        call evlsf(covTest,eigenDummy)
!        Rineq2(:,1:numCorrTest) = matmul(RineqPerm(:,1:numCorrTest),.i.transMatrix)
!        probSub(1) = 1-anordf((wvec(1,1)-meanTest(1,1))/sqrt(covTest(1,1)))
!        
!        do i1=2,numTrans
!            ZS(:,1) = meanTest(:,1)
...
!                do z1=1,i1-1
!!                    
!                    call compute_condMeanVar(z1,numCorrTest,meanTest(:,1),covTest(:,:),ZS(:,1),zmean,zsig2)
!!                    
!                    !draw using inverse probability sampling
!                    lowerbS = (Rineq2(z1,numCorrTest+1) - zmean)/sqrt(zsig2) !lower bound on standardized scale
!                    call inverse_prob_sampling(zmean,zsig2,1,0,wvec(z1,1),dummyUB,ZS(z1,1))
!                end do
!                
!                call compute_condMeanVar(i1,numCorrTest,meanTest(:,1),covTest(:,:),ZS(:,1),zmean,zsig2)
!                
!                !compute conditional probability that Z(i1)>Rineq(i1,dimI+1)
!                probvec(s1) = 1-anordf((wvec(i1,1)-zmean)/sqrt(zsig2))
!                
!                !draw unconstrained
!                dummyDraw = rnnof()
!                ZS(i1,1) = dummyDraw * sqrt(zsig2) + zmean !draw using inverse probability sampling
!                    
!                do z1=i1+1,numCorrTest
!                    
!                    call compute_condMeanVar(z1,numCorrTest,meanTest(:,1),covTest(:,:),ZS(:,1),zmean,zsig2)
!                    
!                    dummyDraw = rnnof()
...
!        end do
!        
!        !now evaluate the remaining 'numI+1'-th to 'numCorrTest'-th inequality constraints using same method as in biems
!        do i1=numTrans+1,numI
!!            write(*,*)i1-1, 'constraints evaluated ->', numI-i1+1, 'to go'
!            posthits = 0
!            ZS = meanTest
!            do s1=1,2*samsize2 !Use twice as much draws because this estimation methos is less efficient
!                do z1=1,numCorrTest
!                    
!                    call compute_condMeanVar(z1,numCorrTest,meanTest(:,1),covTest(:,:),ZS(:,1),zmean,zsig2)
!!
...
!                    upperTrue = 0
!                    lowerTrue = 0
!                    !adjust bounds depending on the inequality constraints for constrained sampling
!                    do ic1=1,i1-1
!                        if(Rineq2(ic1,z1) > machPres) then
!                            lowerbIC = max(lowerbIC,(wvec(ic1,1) - sum(Rineq2(ic1,1:numCorrTest) * ZS(:,1)) &
!                                + ZS(z1,1) * Rineq2(ic1,z1))/Rineq2(ic1,z1))
!                            lowerTrue = 1
!                        else if(Rineq2(ic1,z1) < -machPres) then
!                            upperbIC = min(upperbIC,(wvec(ic1,1) - sum(Rineq2(ic1,1:numCorrTest) * ZS(:,1)) &
!                                + ZS(z1,1) * Rineq2(ic1,z1))/Rineq2(ic1,z1))
!                            upperTrue = 1
!                        end if
!                    end do
!!                   
!                    call inverse_prob_sampling(zmean,zsig2,lowerTrue,upperTrue,lowerbIC,upperbIC,ZS(z1,1))
!                end do
!!               
!                if(sum(Rineq2(i1,1:numCorrTest)*ZS(:,1)) > Rineq2(i1,numCorrTest+1)) then
!                    posthits = posthits + 1
!                end if
!!               
!            end do
!            probSub(i1) = real(posthits)/real(2*samsize2)
!            
!        end do
!        
!        !insert similar algorithm but then by averaging over posterior hits. Perhaps use above method partly.
!    end if
!!
!    probI = product(probSub)
!!
...
!
    ...
    real                :: BDraws(numG,K,P), sigmaDraws(numG,P), CDraws(numG,P,P), SSj(P,P), Wp(P,P),&
                           ...
                           varz1, varz2, varz1z2Plus, varz1z2Min, Cinv(P,P), acceptSigma(numG,P), &
                           detR, covBeta(P*K,P*K), betaDrawj(1,P*K), DummyPP(P,P), &
...
!
800 format(20F8.3)
!    open(60,file='posteriordraws.txt',status='replace')
...
!
            logR_MH_part1 = (-.5*real(nutarget+P+1))*(log(det(Ccan))-log(det(Ccurr))) !target prior
            logR_MH_part2 = -.5*real(nutarget)*(sum(log(diagonals(CcanInv)))-sum(log(diagonals(CcurrInv))))
            logR_MH_part3 = (-.5*real(P+1))*(log(det(Ccurr))-log(det(Ccan))) !proposal prior
            R_MH = exp((1.0-real(JUprior))*(logR_MH_part1+logR_MH_part2)+logR_MH_part3)
!
...
!    
!    deallocate(diffmat)
!    allocate(diffmat(samsize0,numcorr))
!    ones = 1
!    postZmean = matmul(transpose(Zcorr_sample),ones)/real(samsize0)
!    diffmat = Zcorr_sample - matmul(ones,transpose(postZmean))
!    postZcov = (diffmat.tx.diffmat)/real(samsize0)
!    
...
!
    ...
    real :: BDraws(numG,K,P), CDraws(numG,P,P), sigmaDraws(numG,P), SSj(P,P), meanMat(Ntot,P), SigmaMatDraw(P,P), R_MH, &
            ...
            varz1, varz2, varz1z2Plus, varz1z2Min, Cnugget(P,P), &
            Wgroups(numG,Ntot,P), dummyVar, alphaMin, alphaMax, Cinv(P,P), Bmean(K,P), &
            ...
            do p1=1,P
                if(ordinal(p1)>0) then
                    do i1=1,Njs(g1)
                        ...
                        !check if Y falls in largest category, then no upperbound in inverse probability sampling
                        CatUpper = Cat(g1,p1) - Yi1Categorie
                        if(CatUpper>1) then
                           CatUpper = 1
                        end if
...
!
            ...
            logR_MH_part2 = -.5*real(nutarget)*(sum(log(diagonals(CcanInv)))-sum(log(diagonals(CcurrInv))))
            logR_MH_part3 = (-.5*real(P+1))*(log(det(Ccurr))-log(det(Ccan))) !proposal prior
            R_MH = exp((1.0-real(jointunif))*(logR_MH_part1+logR_MH_part2)+logR_MH_part3)
!
            !note that if Wp is not the identity matrix we have to compute sum(diagonals(matmul(Wp,RcurrInv))) in the last line
            call rnun(rnunif)
            if(rnunif(1) < R_MH) then
...
!            write(60,*)CDraws(1,2,1),CDraws(1,3,1),CDraws(1,3,2)
!           
            !draw sigma's
            do p1 = 1,P
                if(ordinal(p1)==0) then
...
!            
        ...
        CDrawsStore(s1,1:numG,1:P,1:P) = CDraws(1:numG,1:P,1:P)
!        
!        write(60,800)CDraws(1,2,1),CDraws(1,3,1),CDraws(1,3,2)
!
    end do
!
    !compute medians Fisher transformed draws
...
!
    ...
    integer              :: irank
    real                 :: chA(P,P), RNmat(nu,P)!, dummyPP(P,P)
!
...
subroutine gibbssampler(k,numER,numIR,rowrank,bet,transcon,B,invbetacov,transR,fcond,Num_k)

   use IMSL_LIBRARIES
   implicit none
   
   integer                          :: N, i, j, time
   integer, intent(in)              :: k, rowrank, numER, numIR
   integer, intent(out)             :: Num_k
!
   real                             :: lower, upper, large, small, p, GG, temp
   real, intent(in)                 :: bet(numER+rowrank), transcon(numIR)
   real, allocatable, dimension(:)  :: gibbsample, conmeans
   real, intent(out)                :: fcond
   real, intent(in)                 :: B(numER+rowrank,numER+rowrank), invbetacov(numER+rowrank,numER+rowrank), &
                                       transR(numIR,numIR)
   
    
    allocate(gibbsample(numER+rowrank),conmeans(numER+rowrank))      
  ...
  777  fcond=0
       ...
       do time=1,N+100 ! "time" denotes the iteration index in the gibbs sampling, 100 denotes the burn-in period.
          ...
          ! "conmeans" is the mean of the conditional posterior normal distribution
             conmeans(i)=bet(i)+sum(B(i,1:numER+rowrank)*(gibbsample(1:numER+rowrank)- &
                             bet(1:numER+rowrank))) - B(i,i)*(gibbsample(i)-bet(i))
                   
             if (i.LE.numER) then
                gibbsample(i)=0
             else if (i<(k+numER)) then ! When computing the probability of beta_{k}>0, beta_{i},i=1,...,k-1 have to be constrained (bigger than 0) 
                           
                ! lower bound is 0, but it has to be standardized.
                lower=(0-conmeans(i))/sqrt(1/invbetacov(i,i)) 
                GG=RNUNF() ! This function generates a pseudorandom number from a uniform (0,1) distribution
                
                ! "ANORDF" returns a probability that a normal random variable takes a value less than or equal to "lower"
                p=ANORDF(lower)+GG*(1-ANORDF(lower))
               
                ! Avoid p=1 and p=0, which are not accepted by "ANORIN" function
                if (p>0.999999) then !mark
                p=0.999999
                end if
                if (p<0.000001) then
                p=0.000001
                ...
                ! "ANORIN" evaluates the inverse of the strandard normal distribution function.
                temp=ANORIN(p)
                ! Gibbs sample of beta_{k} is then obtained
                gibbsample(i)=temp*sqrt(1/invbetacov(i,i))+conmeans(i) 
             else ! if i>k, sample is unconstrained.
                GG=RNNOF()
		        gibbsample(i)=GG*sqrt(1/invbetacov(i,i))+conmeans(i)
             end if  
          ...
          if (time>100) then ! discard first 100 sample         
                temp=conmeans(numER+k)/sqrt(1/invbetacov(numER+k,numER+k)) 
                ! ANORDF(temp) is the probability Pr(beta_{k}>0|beta_{i}^{t}>0,i<k)
                fcond=fcond+ANORDF(temp)/real(N) ! We sum Pr(beta_{k}>0)/N and obtain the mean of Pr(beta_{k}>0).
          end if  
       end do   
             

       ! Reset the number of iterations (sample size T) based on the value of the estimation.
       if (fcond<0.166 .AND. N==3000) then
           N=8000
           go to 777
       else if (fcond<0.0417 .AND. N==8000) then
           N=40000
           go to 777
       else if (fcond<0.00833 .AND. N==40000) then
           N=96000
           go to 777
       else if (fcond<0.00139 .AND. N==96000) then
           N=224000
           go to 777
       else if (fcond<0.000198 .AND. N==224000) then
           N=512000
           go to 777
       else if (fcond<0.0000248 .AND. N==512000) then 
           N=1000000
           go to 777
       end if
       Num_k=N ! final sample size in "k"-th constraint
  ...
  888  fcond=0
                ...
                if (k>(numER+rowrank+1)) then
                    do j=(rowrank+1),(k-1)
                       if (transR(j,i)<0) then
                          large=max(large,(transR(j,i-numER)*gibbsample(i)-sum(transR(j,1:rowrank)* &
                                    gibbsample(numER+1:numER+rowrank))-transcon(j))/transR(j,i-numER))
                       end if
                       if (transR(j,i)>0) then
                          small=min(small,(transR(j,i-numER)*gibbsample(i)-sum(transR(j,1:rowrank)* &
                                    gibbsample(numER+1:numER+rowrank))-transcon(j))/transR(j,i-numER))
                       end if
                    end do
                end if    
                lower=(large-conmeans(i))/sqrt(1/invbetacov(i,i)) 
                upper=(small-conmeans(i))/sqrt(1/invbetacov(i,i)) 
                GG=RNUNF() ! This function generates a pseudorandom number from a uniform (0,1) distribution            
                ! "ANORDF" returns a probability that a normal random variable takes a value less than or equal to "lower"
                p=ANORDF(lower)+GG*(ANORDF(upper)-ANORDF(lower))
               
                ! Avoid p=1 and p=0, which are not accapted by "ANORIN" function
                if (p>0.999999) then
                p=0.999999
                end if
                if (p<0.000001) then
       ...
       if (fcond<0.166 .AND. N==3000) then
           N=9600
           go to 888
       else if (fcond<0.042 .AND. N==9600) then
           N=120000
           go to 888
       else if (fcond<0.008 .AND. N==120000) then
           N=360000
           go to 888
       else if (fcond<0.0014 .AND. N==360000) then 
           N=2520000
           go to 888
       else if (fcond<0.0002 .AND. N==2520000) then  
           N=10000000
           go to 888
       end if
       Num_k=N      
    end if
deallocate(gibbsample,conmeans)
...
!Computation of 'relative fit/complexity', fE and fI|E, based on equality matrix and inequality matrix, resp.
subroutine compute_prob_bain(numER,numIR,dim1,ERr,IRr,theta,thetacov,ffe,ffi)

    use IMSL_LIBRARIES
    implicit none
    
    integer, intent(in)    :: numER, numIR, dim1
    real, intent(in)       :: ERr(numER,dim1+1), IRr(numIR,dim1+1), &
                              theta(dim1+1), thetacov(dim1,dim1)
    real, intent(out)      :: ffe, ffi
    integer	               :: rowrank, i, j, k
    real	               :: constant(numIR), transR(numIR,numIR), ff(numIR), Rr(numER+numIR,dim1+1), &
                              numf(numIR), pi, transcon(numIR)
    real, allocatable	   :: bet(:), betacov(:,:), tempmatrix(:,:), invbetacov(:,:), invDiag(:,:), &
                              IE(:,:), B(:,:)
    
    constant(:) = IRr(:,dim1+1)
!                 
    call Mrank(numIR, dim1, rowrank, IRr, transR, constant, transcon) 
!
    allocate (bet(numER+rowrank),betacov(numER+rowrank,numER+rowrank), &
        tempmatrix(numER+rowrank,dim1),invbetacov(numER+rowrank,numER+rowrank), &
        invDiag(numER+rowrank,numER+rowrank),IE(numER+rowrank,numER+rowrank), &
        B(numER+rowrank,numER+rowrank)) 
!
    do i = 1, numER+rowrank
        if (i.LE.numER) then 
           Rr(i,1:(dim1+1))=ERr(i,1:(dim1+1))
        else 
           Rr(i,1:(dim1+1))=IRr(i-numER,1:(dim1+1))
        end if
    end do
!     
    ! Parameter transformation: beta=Rrtheta-r  
    call MURRV(Rr(1:(numER+rowrank), 1:(dim1+1)), theta(1:(dim1+1)), bet) ! beta=Rr*theta  
    call MRRRR(Rr(1:(numER+rowrank), 1:dim1), thetacov, tempmatrix)  ! tempmatrix=R*thetacov. Note that R=Rr(1:numR,1:numcorr): discard the last column. 
    call MXYTF(tempmatrix, Rr(1:(numER+rowrank),1:dim1), betacov)   ! betacov=tempmatrix*R^(T), then betacov=R*cov*R^(T)
!        
    pi=3.1415926
!            
    call LINRG(betacov,invbetacov)        ! Compute the inverse of betacov i.e., invbetacov=betacov^(-1)
...
!
    ...
    deallocate(bet,betacov,tempmatrix,invbetacov,invDiag,IE,B) 

end subroutine compute_prob_bain
